...
class Archer:
    def __init__(this):
        this.__total_health = int(Barbarians().ret_health() / 2) # half of health of the barbarian
        this.__health = int(Barbarians().ret_health() / 2)
        this.__isdead = 0
        this.__speed = Barbarians().ret_speed() * 2 # twice of barbarian
        this.__damage = int(Barbarians().ret_health() / 2) # half of barbarbarian
        this.__row = game_ht[1] - 2
        this.__col = 0
        this.__is_started = 0
        this.__active = 0
        this.__archer = "A" # representing the archer
    ...
    def ret_archer(this):
        return this.__archer

    def upd_archer(this,x):
        this.__archer = x
    ...
    def reset(this):
        this.upd_total_health(int(Barbarians().ret_health() / 2))
        this.upd_health(int(Barbarians().ret_health() / 2))
        this.upd_row(game_ht[1] - 1)
        this.upd_col(0)
        this.upd_isdead(0)
        this.upd_speed(Barbarians().ret_speed() * 2)
        this.upd_damage(int(Barbarians().ret_health() / 2))
        this.upd_archer("A")
